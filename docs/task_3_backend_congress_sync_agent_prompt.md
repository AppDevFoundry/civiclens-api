...
Implement the **Phase 1 Backend Congress Sync** described in @docs/PROJECT_OVERVIEW_AND_ROADMAP.md. With the Congress.gov API client already in place (`src/app/services/congress/**`), this task wires up a repeatable ingestion pipeline that stores bill data inside `civiclens-api/`, keeps it fresh via a cron-style job, and exposes read-friendly endpoints that the existing RealWorld-based clients can consume (web work can follow later).
